...
// Check if the user is logged in and has the correct role (Super Admin)
if (!isset($_SESSION['user_id']) || $_SESSION['role_name'] !== 'Super Admin') {
    echo "<script>
            alert('Super admin access only!');
            window.location.href = '../public/dashboard/admin.php'; 
        </script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $codeQuery = $conn->prepare("SELECT * FROM superadmin_codes WHERE code = ? AND status = 'unused'"); // Change table name to superadmin_codes
    ...
    if ($codeResult->num_rows > 0) {
        ...
        // Set role_id to Admin's role ID, change 5 to Admin's role ID (check your DB)
        $role_id = 4; // Example: Assuming '4' is the Admin role_id
        ...
        if ($insertUser->execute()) {
            ...
            $updateCode = $conn->prepare("UPDATE superadmin_codes SET status = 'used' WHERE code = ?"); // Change to superadmin_codes table
            $updateCode->bind_param("s", $code);
            $updateCode->execute();

            $message = "<script>alert('Admin registration successful!')</script>.";
...
<body>
    <!-- Navigation -->
    <div id="navi">
        <span class="reminder" style="color: white; font-size:2rem; font-weight: bold;">Admin Registration</span>
